...
from mmstn.data.loader import data_loader
from mmstn.losses import gan_g_loss, gan_d_loss, l2_loss, TripletLoss
from mmstn.losses import displacement_error, final_displacement_error

from mmstn.models import TrajectoryGenerator, TrajectoryDiscriminator
from mmstn.utils import int_tuple, bool_flag, get_total_norm
from mmstn.utils import relative_to_abs, get_dset_path
...
# Dataset options
parser.add_argument('--dataset_name', default='datasetsWithMe_name_date', type=str)
...
def main(args):
    ...
    viz = Visdom()
    # 创建窗口并初始化
    my_win = viz.line([[0., 0.]], [0], win='G_loss&D_loss_o13',
                      opts=dict(title='G_loss&D_loss_o13', legend=['G_loss', 'D_loss']))
...
def discriminator_step(
    ...
    (obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel, non_linear_ped,
     loss_mask, seq_start_end,obs_traj_Me, pred_traj_gt_Me, obs_traj_rel_Me, pred_traj_gt_rel_Me) = batch
    losses = {}
    ...
    scores_fake,future_fake = discriminator(traj_fake, traj_fake_rel, seq_start_end)
    scores_real,future_real = discriminator(traj_real, traj_real_rel, seq_start_end)
    # random.randint(0,)
    anchor_list = []
    pos_list = []
    # for _ in range(future_fake.size(0)):
    #     choise = random.sample(range(future_fake.size(0)), 2)
    #     if choise[0]==choise[1]:
    #         choise[1] = choise[0]+1
    #     anchor_list.append(choise[0])
    #     pos_list.append(choise[1])
    # anchor = future_real[anchor_list,:]
    # pos = future_real[pos_list,:]
    # trloss = tripletLoss(anchor,pos,future_fake)
    # losses['trip_loss'] = trloss.item()
    # loss +=trloss
...
def generator_step(
    ...
    scores_fake,_ = discriminator(traj_fake, traj_fake_rel, seq_start_end)
...
def check_accuracy(
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            scores_fake,_ = discriminator(traj_fake, traj_fake_rel, seq_start_end)
            scores_real,_ = discriminator(traj_real, traj_real_rel, seq_start_end)
